...
CREATE TABLE IF NOT EXISTS admin_pin (id INTEGER PRIMARY KEY CHECK(id = 1),pin_hash TEXT NOT NULL,salt TEXT NOT NULL,updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS product_categories (id TEXT PRIMARY KEY,name TEXT UNIQUE NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS products (id TEXT PRIMARY KEY,category_id TEXT NOT NULL,name TEXT NOT NULL,formulation_type TEXT,active_ingredient TEXT,concentration TEXT,application_method TEXT,repeat_interval TEXT,key_benefits TEXT,mode_of_action TEXT,compatibility TEXT,unique_strain_info TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,FOREIGN KEY(category_id) REFERENCES product_categories(id));
